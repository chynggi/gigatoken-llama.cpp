 ...
 *
 * Upstream already covers JSON export/import (`conversationsStore.downloadConversation`,
 * `importConversationsData`, and the settings import/export tab), so only these
 * two document formats live in the fork.
 *
 * Both formatters take the export timestamp as a parameter rather than reading
 * the clock, so their output is reproducible and can be asserted in tests.
 */
 ...
 *
 * @param conv - The conversation being exported
 * @param messages - Its messages, in display order; `root` entries are skipped
 * @param exportedAt - Timestamp stamped into the header
 */
export function exportConversationAsMarkdown(
	...
	lines.push('');
	lines.push(`_Exported on ${exportedAt.toLocaleString()}_`);
	lines.push('');
...
export function exportConversationAsHtml(
	...
	bodyLines.push(`<p class="export-date">Exported on ${exportedAt.toLocaleString()}</p>`);
