...
import { SearchProviderService } from '$lib/fork/packs/search-provider.service';
...
class SearchProvidersStore {
	...
	get enabled(): boolean {
		return Boolean(settingsStore.config[FORK_SETTINGS_KEYS.WEB_SEARCH_ENABLED]);
	}

	get activeProviderId(): string {
		return String(settingsStore.config[FORK_SETTINGS_KEYS.WEB_SEARCH_ACTIVE_PROVIDER] || '');
	}

	get autoDetect(): boolean {
		return Boolean(settingsStore.config[FORK_SETTINGS_KEYS.WEB_SEARCH_AUTO_DETECT]);
	}
	...
	async search(query: string): Promise<SearchResult[]> {
		...
		try {
			const providerConfig: SearchProviderConfig = {
				...
				resultsCount:
					Number(settingsStore.config[FORK_SETTINGS_KEYS.WEB_SEARCH_RESULTS_COUNT]) || 5
			};
